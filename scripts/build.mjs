...
export async function build({ root = project, output = path.join(root, 'dist') } = {}) {
  const data = {};
  for (const name of ['site', 'home', 'programs', 'committees', 'board', 'events']) {
  ...
  const { site, home, programs, committees, board, events } = data;
...
<footer><div class="footer-grid"><section><h2>${escape(site.footer.contactTitle)}</h2>${link(site.email, `mailto:${site.email}`)}<h2>${escape(site.footer.socialTitle)}</h2><div class="socials">${site.socials.map(s => `<a href="${escape(s.href)}" aria-label="${escape(s.label)}">${img(s.image,s.label)}</a>`).join('')}</div></section><section class="newsletter"><h2>${escape(site.newsletter.title)}</h2><p>${escape(site.newsletter.description)}</p>${link(site.newsletter.label,site.newsletter.href,'button outline')}<p class="join-help">${escape(site.newsletter.joinHelp)}</p></section></div><div class="footer-bottom"><p class="copyright">© ${escape(site.footer.copyright)}</p><p class="updated">${escape(site.footer.updatedLabel)} <time datetime="${escape(commitDate)}">${escape(displayDate)}</time></p></div></footer></body></html>`;
    return html;
  }
  const pages = {
    '': render('', home.title, ({img,link,url}) => `
    ...
    'programs/': render('programs/',programs.title,({img,link}) => `
    ...
    'committees/': render('committees/',committees.title,({img,link,url})=>`
